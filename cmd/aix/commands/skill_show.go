...
func init() {
	skillShowCmd.Flags().BoolVar(&showJSON, "json", false, "Output as JSON")
	skillShowCmd.Flags().BoolVar(&showFull, "full", false, "Show complete instructions (default truncated)")
...
// showSkillDetail holds unified skill information for display.
type showSkillDetail struct {
...
func runSkillShow(_ *cobra.Command, args []string) error {
	...
	for _, p := range platforms {
		...
		if detail == nil {
			detail = extractSkillDetail(skillAny)
		}
	}
	...
	return outputShowText(detail)
}

// extractSkillDetail converts a platform-specific skill to the unified detail struct.
func extractSkillDetail(skill any) *showSkillDetail {
	switch s := skill.(type) {
	case *claude.Skill:
		return extractClaudeSkill(s)
	case *opencode.Skill:
		return extractOpenCodeSkill(s)
	default:
		return nil
	}
}
...
// extractOpenCodeSkill extracts details from an OpenCode skill.
func extractOpenCodeSkill(s *opencode.Skill) *showSkillDetail {
...
func outputShowText(detail *showSkillDetail) error {
	fmt.Printf("Skill: %s\n", detail.Name)

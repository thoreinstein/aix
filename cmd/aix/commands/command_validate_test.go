...
func TestFormatCommandParseError(t *testing.T) {
	tests := []struct {
		name    string
		err     error
		wantMsg string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := formatCommandParseError(tt.err)
			if got != tt.wantMsg {
				t.Errorf("formatCommandParseError() = %q, want %q", got, tt.wantMsg)
			}
		})
	}
}

func TestCommandValidateCommand_Metadata(t *testing.T) {
	if commandValidateCmd.Use != "validate <path>" {
		t.Errorf("Use = %q, want %q", commandValidateCmd.Use, "validate <path>")
	}

	if commandValidateCmd.Short == "" {
		t.Error("Short description should not be empty")
	}

	if commandValidateCmd.Args == nil {
		t.Error("Args validator should be set")
	}
}

func TestRunCommandValidate_ValidCommand(t *testing.T) {
...
`
	...
	err := runCommandValidate(cmdPath, &out)

	if err != nil {
		t.Errorf("runCommandValidate() unexpected error: %v", err)
	}
...
func TestRunCommandValidate_InvalidCommand(t *testing.T) {
...
`
	...
	if err != errCommandValidationFailed {
		t.Errorf("runCommandValidate() error = %v, want errCommandValidationFailed", err)
	}
...
func TestRunCommandValidate_MissingFile(t *testing.T) {
	var out bytes.Buffer
	err := runCommandValidate("/nonexistent/path/command.md", &out)
...
func TestRunCommandValidate_NameInferredFromFilename(t *testing.T) {
...
func TestRunCommandValidate_JSONOutput(t *testing.T) {
	// Create a valid command file
	tempDir := t.TempDir()
	cmdPath := filepath.Join(tempDir, "review.md")
...
`
	...
	// Set JSON flag
	oldValue := commandValidateJSON
	commandValidateJSON = true
	defer func() { commandValidateJSON = oldValue }()
...
func TestFormatCommandValidationIssue(t *testing.T) {
...
func TestCommandValidate_StrictFlag(t *testing.T) {
	// Test that the strict flag is available
	flag := commandValidateCmd.Flags().Lookup("strict")
	if flag == nil {
		t.Error("--strict flag should be defined")
	}
}

func TestCommandValidate_JSONFlag(t *testing.T) {
	// Test that the json flag is available
	flag := commandValidateCmd.Flags().Lookup("json")
	if flag == nil {

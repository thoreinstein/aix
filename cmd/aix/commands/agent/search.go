...
// searchResultJSON represents an agent search result for JSON output.
...
// searchCmd is the agent search subcommand.
var searchCmd = &cobra.Command{
	Use:   "search <query>",
	Short: "Search for agents in repositories",
	Long:  `Search for agents across all configured repositories by name or description.`,
	Example: `  # Search for agents matching "test"
...
// runSearch executes the agent search command.
...
func runSearchWithWriter(w io.Writer, args []string) error {
	...
	// Build search options - filter to agents only
	...
	if len(results) == 0 {
		fmt.Fprintf(w, "No agents found matching %q\n", query)
		return nil
	}
...
func outputSearchJSON(w io.Writer, results []resource.Resource) error {
	...
	enc.SetIndent("", "  ")
	return errors.Wrap(enc.Encode(jsonResults), "encoding search results")
}

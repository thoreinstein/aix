...
// mcpRemoveMockPlatform extends mockPlatform for MCP remove testing.
type mcpRemoveMockPlatform struct {
	mockPlatform
...
func (m *mcpRemoveMockPlatform) GetMCP(name string) (any, error) {
...
func (m *mcpRemoveMockPlatform) RemoveMCP(_ string) error {
	m.removeCalled = true
	return m.removeErr
}
...
func TestConfirmMCPRemoval(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: "yes\n",
			platforms: []cli.Platform{
				&mcpRemoveMockPlatform{mockPlatform: mockPlatform{displayName: "Claude Code"}},
			},
			want: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := confirmMCPRemoval(&out, in, "github", tt.platforms)
			if got != tt.want {
...
func TestConfirmMCPRemoval_ListsPlatforms(t *testing.T) {
	platforms := []cli.Platform{
		&mcpRemoveMockPlatform{mockPlatform: mockPlatform{displayName: "Claude Code"}},
		&mcpRemoveMockPlatform{mockPlatform: mockPlatform{displayName: "OpenCode"}},
	}

	var out bytes.Buffer
	in := strings.NewReader("n\n")

	confirmMCPRemoval(&out, in, "github", platforms)
...
func TestMCPRemoveCommand_Metadata(t *testing.T) {
	if mcpRemoveCmd.Use != "remove <name>" {
		t.Errorf("Use = %q, want %q", mcpRemoveCmd.Use, "remove <name>")
	}

	if mcpRemoveCmd.Short == "" {
		t.Error("Short description should not be empty")
	}

	if mcpRemoveCmd.Args == nil {

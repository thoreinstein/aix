...
func TestSearchCommand_NoResults(t *testing.T) {
	...
	// Create a repo directory with commands dir (but no commands)
	...
	var buf bytes.Buffer
	err := runSearchWithWriter(&buf, []string{"nonexistent-command"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	output := buf.String()
	if !strings.Contains(output, `No commands found matching "nonexistent-command"`) {
		t.Errorf("expected 'No commands found matching' message, got:\n%s", output)
	}
}
...
func TestSearchCommand_ValidRepoFilter(t *testing.T) {
	...
	// Create a repo directory with a command
	repoDir := filepath.Join(tmpDir, "repos", "my-repo")
	commandDir := filepath.Join(repoDir, "commands", "test-command")
	if err := os.MkdirAll(commandDir, 0o755); err != nil {
		t.Fatalf("failed to create command dir: %v", err)
	}

	// Write a valid command.md
	commandContent := `---
name: test-command
description: A test command for searching
---
This is a test command.
`
	if err := os.WriteFile(filepath.Join(commandDir, "command.md"), []byte(commandContent), 0o644); err != nil {
		t.Fatalf("failed to write command.md: %v", err)
	}
	...
	output := buf.String()
	// Should find the command - check for command name in output
	if !strings.Contains(output, "test-command") {
		t.Errorf("expected to find 'test-command' in output, got:\n%s", output)
	}
}
...
func TestSearchCommand_JSONOutput(t *testing.T) {
	...
	commandDir := filepath.Join(repoDir, "commands", "json-command")
	if err := os.MkdirAll(commandDir, 0o755); err != nil {
		t.Fatalf("failed to create command dir: %v", err)
	}
	...
	commandContent := `---
name: json-command
description: A command for JSON output testing
---
This is a test command.
`
	if err := os.WriteFile(filepath.Join(commandDir, "command.md"), []byte(commandContent), 0o644); err != nil {
	...
	if !strings.Contains(output, `"name": "json-command"`) {
		t.Errorf("expected JSON output with command name, got:\n%s", output)
	}

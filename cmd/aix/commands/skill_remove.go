...
func init() {
	skillRemoveCmd.Flags().BoolVar(&removeForce, "force", false, "Skip confirmation prompt")
...
func runSkillRemove(_ *cobra.Command, args []string) error {
	return runSkillRemoveWithIO(args, os.Stdout, os.Stdin)
}

// runSkillRemoveWithIO allows injecting writers for testing.
func runSkillRemoveWithIO(args []string, w io.Writer, r io.Reader) error {
	name := args[0]

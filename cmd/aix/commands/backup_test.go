...
func TestBackupRestore_RequiresPlatform(t *testing.T) {
	...
	defer func() { platformFlag = origPlatformFlag }()
	platformFlag = nil // No platform specified

	var buf bytes.Buffer
	err := runBackupRestoreWithWriter(nil, nil, &buf)
	if err == nil {
...
func TestBackupPrune_NegativeKeep(t *testing.T) {
	origKeep := backupPruneKeep
	defer func() { backupPruneKeep = origKeep }()
	backupPruneKeep = -1

	var buf bytes.Buffer
	err := runBackupPruneWithWriter(&buf)
	if err == nil {

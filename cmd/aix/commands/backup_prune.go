...
func init() {
	backupPruneCmd.Flags().IntVar(&backupPruneKeep, "keep", backup.DefaultRetentionCount,
...
func runBackupPrune(_ *cobra.Command, _ []string) error {
	return runBackupPruneWithWriter(os.Stdout)
}

func runBackupPruneWithWriter(w io.Writer) error {
	if backupPruneKeep < 0 {
	...
	for _, p := range platforms {
		...
		toRemove := len(manifests) - backupPruneKeep
		if toRemove <= 0 {
			continue
		}

		if err := mgr.Prune(p.Name(), backupPruneKeep); err != nil {

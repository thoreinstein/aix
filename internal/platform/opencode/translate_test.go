...
func TestValidateVariables(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "repeated unknown variable deduplicated",
		...
		{
			name:    "mixed case is not a variable",
			content: "$Arguments is not matched",
			wantErr: false,
		},
	}
...
func TestListVariables(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			content: "$MY_VAR and $ANOTHER_VAR",
			want:    []string{"$MY_VAR", "$ANOTHER_VAR"},
		},
	}
...
func TestVarPattern(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "does not match numbers in variable",
			input:   "$VAR123",
			matches: nil,
		},
		{
		...
		{
			name:    "matches variable at start of line",
			input:   "$ARGUMENTS at start",
			matches: []string{"$ARGUMENTS"},
		},
		{
			name:    "matches variable at end of line",
			input:   "at end $ARGUMENTS",
			matches: []string{"$ARGUMENTS"},
		},
		{
			name:    "single letter after $ not matched",
			input:   "$A",
			matches: nil,
		},
		{
			name:    "two uppercase letters minimum",
			input:   "$AB",
			matches: []string{"$AB"},
		},
	}
